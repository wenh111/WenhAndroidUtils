...
public final class HttpLogInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        log.append(requestStartMessage).append("\n");
        if (logHeaders) {
            if (hasRequestBody) {
            ...
            if (!logBody || !hasRequestBody) {
                ...
                if (requestBody.contentLength() < 10240)
        ...
        String write = "<-- " + response.code() + ' ' + response.message() + ' '
                + response.request().url() + " (" + tookMs + "ms" + (!logHeaders ? ", "
                + bodySize + " body" : "") + ')';
        logger.log(write);
        log.append(write).append("\n");
        if (logHeaders) {
        ...
        com.wenh.baselibrary.LoggerWindow.Logger.i(log.toString());
//		logger.log(write);
        return response;
    }
